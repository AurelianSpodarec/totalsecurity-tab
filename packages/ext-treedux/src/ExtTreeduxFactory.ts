...
export class ExtTreeduxFactory<DataStoreMap extends DefaultDataStoreMap = DefaultDataStoreMap> {
  ...
  public bootstrap(): void {
    if (!Context.isBackground())
      throw new Error(
        `bootstrap() method has been called from a non-background context for Treedux store "${this.name}"`
      );
  ...
  private async initStore(): Promise<ExtTreedux<DataStoreMap>> {
    if (!this.instance) {
      ...
      const { [stateStorageKey]: state, [overridesStorageKey]: overrides } = await LocalStorageApi.get(
        stateStorageKey,
        overridesStorageKey
      );
      this.instance = ExtTreedux.init(this.name, this.dataStores, { initialState: state, overrides });
    }
